...
let row = document.querySelector('.form__row-submit');
let users: User[] = [];
let dniUserUpdate: String;
let url: String = 'https://task-crud-users.herokuapp.com'
class User {
  name: String;
  lastName: String;
  email: String;
...
function onClickSaveUser() {
  let form: any = document.querySelector('.register-users');
  let user: User = {
...
async function addUser(user: User) {
  if (dniUserUpdate !== undefined) {
    await updateUser(dniUserUpdate, user);
  } else {
    await fetch(`${url}/users/`, {
      method: 'POST',
      body: JSON.stringify(
        { "names": user.name, "last_names": user.lastName, "email": user.email, "dni": user.dni }
      ),
...
function onClickUpdateUser(event: any) {
  let form: any = document.querySelector('.register-users');
  let i = 0;
  while (++i < 5) {
...
function onClickDeleteUser(event: any) {
...
function deleteUser(dni: String) {
  fetch(`${url}/users/${dni}`, {
    method: 'DELETE',
    headers: { 'Content-Type': 'application/json' }
  })
...
function isEmpty(check: String): boolean {
  return check.length == 0;
}

function cleanShowUsers() {
  let showUsers: any = document.querySelector('.show-users');
  while (showUsers.firstChild) {
    showUsers.removeChild(showUsers.firstChild)
  }
}

function getDni(event: any) {
